...
impl SpiNode {
    ...
    pub fn open(&mut self) {
        let os_dev = unsafe { mynewt_core_hw_bus_bindgen::os_dev_open(self.dev_name.as_ptr(), 0, core::ptr::null_mut()) };
    ...
    pub fn write(&mut self, tx_buf: &[u8]) -> Result<(), i32> {
        ...
        let result = unsafe { mynewt_core_kernel_os_bindgen::os_time_ms_to_ticks(timeout, &mut ticks) };
        assert!(result == 0);

        let result = unsafe { mynewt_core_hw_bus_bindgen::bus_node_write(self.os_dev, tx_buf.as_ptr() as *const core::ffi::c_void, tx_buf.len() as u16, ticks, mynewt_core_hw_bus_bindgen::BUS_F_NONE as u16) };
