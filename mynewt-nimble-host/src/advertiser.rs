...
pub struct BleAdvertiser {
    own_addr_type: u8,
    adv_params: Option<mynewt_nimble_host_bindgen::ble_gap_adv_params>,
    fields: Option<mynewt_nimble_host_bindgen::ble_hs_adv_fields>,
}
...
impl BleAdvertiser {
    ...
    fn bleprph_advertise() {
        ...
        // let name = mynewt_nimble_host_bindgen::ble_svc_gap_device_name();
        let name = "pinetime";
        fields.name = name.as_ptr();
        fields.name_len = name.len() as u8;
        fields.set_name_is_complete(1);

        let rc = unsafe { mynewt_nimble_host_bindgen::ble_gap_adv_set_fields(fields) };
        assert!(rc == 0);
